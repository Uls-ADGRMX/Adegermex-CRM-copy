  ...
  <tr>
    <td bgcolor="#FFFFFF" align="center"><br />
      <table width="900" border="0" cellspacing="0" cellpadding="4">
        <tr>
          <td align="center" class="titulo">Proyectos por Tipo de Aprobación</td>
        </tr>
        <tr>
...
$aprobados=mysql_query("SELECT DISTINCT tmproyectos.aprobacion
FROM tmproyectos WHERE tmproyectos.aprobacion<>'' ORDER BY tmproyectos.aprobacion DESC",$conexion);
while($fila=mysql_fetch_array($aprobados)){
	$ap = $fila['aprobacion'];
	$proyectos = mysql_query("
	SELECT tmproyectos.*, tcclientes.nombre AS cliente
	FROM tmproyectos
	JOIN tcclientes
	WHERE tmproyectos.id_cliente = tcclientes.id_cliente AND tmproyectos.aprobacion = '$ap' AND tmproyectos.fecha_generacion BETWEEN '$fecha_inicial' AND '$fecha_final' ORDER BY id_proyecto ASC",$conexion);
