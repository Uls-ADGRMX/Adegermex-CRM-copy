  ...
  <tr>
    <td bgcolor="#FFFFFF" align="center"><br />
      <table width="900" border="0" cellspacing="0" cellpadding="4">
        <tr>
          <td align="center" class="titulo">Proyectos por Agente de Ventas conteo por Status</td>
        </tr>
        <tr>
...
$totalgeneral = mysql_query("SELECT COUNT(*) AS totalg FROM tmproyectos WHERE tmproyectos.fecha_generacion BETWEEN '$fecha_inicial' AND '$fecha_final'",$conexion);
$filatotalg = mysql_fetch_array($totalgeneral);
$totalg = $filatotalg['totalg'];
$agentes=mysql_query("SELECT DISTINCT tcusuarios.nombre, tcusuarios.id_usuario
FROM tcusuarios
JOIN tmproyectos
WHERE tcusuarios.id_usuario = tmproyectos.id_usugenera ORDER BY tcusuarios.nombre ASC",$conexion);
while($fila=mysql_fetch_array($agentes)){
	$us = $fila['id_usuario'];
	$proyectos = mysql_query("
	SELECT DISTINCT tmproyectos.status, COUNT(tmproyectos.status) AS numero_proyectos
	FROM tmproyectos
	WHERE tmproyectos.id_usugenera='$us' AND tmproyectos.fecha_generacion BETWEEN '$fecha_inicial' AND '$fecha_final' GROUP BY tmproyectos.status ORDER BY numero_proyectos DESC",$conexion);
	$nproyectos = mysql_num_rows($proyectos);
	echo '
	<table width="950" border="0" cellspacing="0" cellpadding="4">
		<tr>
			<td class="factura-texto4" align="center" bgcolor="#EFEFEF">'.$fila['nombre'].'</td>
		</tr>
        <tr>
          <td><br/>';
if ($nproyectos=="0")
{
	...
	FROM tmproyectos
	WHERE tmproyectos.id_usugenera='$us' AND tmproyectos.fecha_generacion BETWEEN '$fecha_inicial' AND '$fecha_final'",$conexion);
	$filatotal = mysql_fetch_array($total);
	$totalp = $filatotal['totalp'];
		  echo '<table width="600" border="0" align="center" cellpadding="4" cellspacing="0">
			<tr class="encabezado-tabla">
				<td width="300">Status</td>
				<td width="200">Número de Proyectos</td>
				<td width="100">Porcentaje</td>
			</tr>';
			while($proyecto=mysql_fetch_array($proyectos)){
				echo '
				<tr>
					<td colspan="3"><img src="imagenes/linea-950.png" width="600" height="1" /></td>
				</tr>
				...
				echo '">'.$proyecto['status'].'</span></td>
					<td>'.$proyecto['numero_proyectos'].' proyectos</td>
					<td>'.number_format($proyecto['numero_proyectos'] * 100 / $totalp,2,".",",").'%</td>
				</tr>';
				if($proyecto['status']=="Finalizado")
				{
					$pfinalizados = mysql_query("
					SELECT DISTINCT tmproyectos.cierre_venta, COUNT(tmproyectos.cierre_venta) AS numero_finalizados
					FROM tmproyectos
					WHERE tmproyectos.id_usugenera='$us' AND tmproyectos.fecha_generacion BETWEEN '$fecha_inicial' AND '$fecha_final' AND tmproyectos.cierre_venta<>'' GROUP BY tmproyectos.cierre_venta ORDER BY numero_finalizados DESC",$conexion);
					while($finalizados=mysql_fetch_array($pfinalizados)){
						echo '
						<tr>
							<td colspan="3"><img src="imagenes/linea-950.png" width="600" height="1" /></td>
					</tr>
					<tr class="celda-activa2">
						<td>
							&emsp;&emsp;&emsp;|--- ';
							switch ($finalizados['cierre_venta']) {
								case "0":
									echo "No Vendido";
									break;
								case "1":
						...
						<td>
							&emsp;&emsp;&emsp;'.$finalizados['numero_finalizados'].' proyectos
						</td>
						<td>
							&nbsp;
						</td>
			...
			echo '
			<tr>
				<td colspan="3" align="right"><img src="imagenes/linea-950.png" width="600" height="1" /></td>
			</tr>
			<tr class="encabezado-tabla">
				<td>Total:</td>
				<td>'.$totalp.' proyectos</td>
				<td>&nbsp;</td>
			</tr>
		</table>';
}
...
echo '<br/><strong>'.$totalg.'</strong> proyectos en total en el periodo.<br/>';
}
?>
<br /></td>
  </tr>
