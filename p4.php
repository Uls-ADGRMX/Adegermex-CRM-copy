  ...
  <tr>
    <td bgcolor="#FFFFFF" align="center"><br />
      <table width="900" border="0" cellspacing="0" cellpadding="4">
        <tr>
          <td align="center" class="titulo">Proyectos por Potencial de Negocio</td>
        </tr>
        <tr>
...
$potenciales=mysql_query("SELECT DISTINCT tmproyectos.potencial
FROM tmproyectos ORDER BY tmproyectos.potencial ASC",$conexion);
while($fila=mysql_fetch_array($potenciales)){
	$pt = $fila['potencial'];
	$proyectos = mysql_query("
	SELECT tmproyectos.*, tcclientes.nombre AS cliente
	FROM tmproyectos
	JOIN tcclientes
	WHERE tmproyectos.id_cliente = tcclientes.id_cliente AND tmproyectos.potencial = '$pt' AND tmproyectos.fecha_generacion BETWEEN '$fecha_inicial' AND '$fecha_final' ORDER BY id_proyecto ASC",$conexion);
