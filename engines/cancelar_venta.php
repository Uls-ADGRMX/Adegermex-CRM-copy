...
$fecha=date("Y-m-d");
$hora=date("H:i:s");
$modificar = mysql_query("UPDATE tmproyectos SET cierre_venta='0', new_win='0', fecha_termino='$fecha', hora_termino='$hora' WHERE id_proyecto='$id_proyecto'", $conexion);
		if (!$modificar) {
			echo '<script language="javascript">alert("Cation : Proyectos\n\nError al cancelar la venta del Proyecto")</script>';
			echo "<script language='javascript'>window.location='../proyectos.php'</script>";
			die("Fallo en la insercion de registro en la Base de Datos: " . mysql_error());
			exit();
			}
$comentario = "<strong>Motivo de la cancelación de la venta</strong>: ".$comentarios;
...
$evento = "Se <strong>Canceló la Venta</strong> del proyecto, lo indicó ";
...
echo '<script language="javascript">alert("Cation : Proyectos\n\nLa venta del proyecto se canceló correctamente.")</script>';
