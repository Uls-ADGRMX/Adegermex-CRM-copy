  ...
  <tr>
    <td bgcolor="#FFFFFF" align="center"><br />
      <table width="900" border="0" cellspacing="0" cellpadding="4">
        <tr>
          <td align="center" class="titulo">Proyectos por Tipo de Proyecto</td>
        </tr>
        <tr>
...
$tipos=mysql_query("SELECT DISTINCT tmproyectos.tipo
FROM tmproyectos ORDER BY tmproyectos.tipo ASC",$conexion);
while($fila=mysql_fetch_array($tipos)){
	$tp = $fila['tipo'];
	$proyectos = mysql_query("
	SELECT tmproyectos.*, tcclientes.nombre AS cliente
	FROM tmproyectos
	JOIN tcclientes
	WHERE tmproyectos.id_cliente = tcclientes.id_cliente AND tmproyectos.tipo = '$tp' AND tmproyectos.fecha_generacion BETWEEN '$fecha_inicial' AND '$fecha_final' ORDER BY id_proyecto ASC",$conexion);
	$nproyectos = mysql_num_rows($proyectos);
	echo '
	<table width="950" border="0" cellspacing="0" cellpadding="4">
		<tr>
			<td class="factura-texto4" align="center" bgcolor="#EFEFEF">'.$fila['tipo'].'<br/><span class="subtitulo">'.$nproyectos.' proyectos en total</span></td>
		</tr>
        <tr>
          <td><br/>';
if ($nproyectos=="0")
