  ...
  <tr>
    <td bgcolor="#FFFFFF" align="center"><br />
      <table width="900" border="0" cellspacing="0" cellpadding="4">
        <tr>
          <td align="center" class="titulo">Proyectos por Agente de Ventas</td>
        </tr>
        <tr>
...
while($fila=mysql_fetch_array($agentes)){
	$us = $fila['id_usuario'];
	$proyectos = mysql_query("SELECT tmproyectos.*, tcclientes.id_cliente, tcclientes.nombre AS cliente FROM tmproyectos JOIN tcclientes WHERE tmproyectos.id_cliente = tcclientes.id_cliente AND id_usugenera = $us AND tmproyectos.fecha_generacion BETWEEN '$fecha_inicial' AND '$fecha_final' ORDER BY tmproyectos.status ASC",$conexion);
	$nproyectos = mysql_num_rows($proyectos);
	echo '
	<table width="950" border="0" cellspacing="0" cellpadding="4">
		<tr>
			<td class="factura-texto4" align="center" bgcolor="#EFEFEF">'.$fila['nombre'].'<br/><span class="subtitulo">'.$nproyectos.' proyectos en total</span></td>
		</tr>
        <tr>
          <td><br/>';
if ($nproyectos=="0")
