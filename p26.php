  ...
  <tr>
    <td bgcolor="#FFFFFF" align="center"><br />
      <table width="900" border="0" cellspacing="0" cellpadding="4">
        <tr>
          <td align="center" class="titulo">Proyectos por Tipo de No Aprobación</td>
        </tr>
        <tr>
...
$noaprobados=mysql_query("SELECT DISTINCT tmproyectos.noaprobacion
FROM tmproyectos WHERE tmproyectos.noaprobacion<>'' ORDER BY tmproyectos.noaprobacion DESC",$conexion);
while($fila=mysql_fetch_array($noaprobados)){
	$nap = $fila['noaprobacion'];
	$proyectos = mysql_query("
	SELECT tmproyectos.*, tcclientes.nombre AS cliente
	FROM tmproyectos
	JOIN tcclientes
	WHERE tmproyectos.id_cliente = tcclientes.id_cliente AND tmproyectos.noaprobacion = '$nap' AND tmproyectos.fecha_generacion BETWEEN '$fecha_inicial' AND '$fecha_final' ORDER BY id_proyecto ASC",$conexion);
	$nproyectos = mysql_num_rows($proyectos);
	echo '
	<table width="950" border="0" cellspacing="0" cellpadding="4">
		<tr>
			<td class="factura-texto4" align="center" bgcolor="#EFEFEF">';
switch ($fila['noaprobacion']){
	...
	echo '<br/><span class="subtitulo">'.$nproyectos.' proyectos en total</span></td>
		</tr>
        <tr>
          <td><br/>';
if ($nproyectos=="0")
