  ...
  <tr>
    <td bgcolor="#FFFFFF" align="center"><br />
      <table width="900" border="0" cellspacing="0" cellpadding="4">
        <tr>
          <td align="center" class="titulo">Proyectos por Cierre de Venta</td>
        </tr>
        <tr>
...
$cierres=mysql_query("SELECT DISTINCT tmproyectos.cierre_venta
FROM tmproyectos ORDER BY tmproyectos.cierre_venta DESC",$conexion);
while($fila=mysql_fetch_array($cierres)){
	$cr = $fila['cierre_venta'];
	$proyectos = mysql_query("
	SELECT tmproyectos.*, tcclientes.nombre AS cliente
	FROM tmproyectos
	JOIN tcclientes
	WHERE tmproyectos.id_cliente = tcclientes.id_cliente AND tmproyectos.cierre_venta = '$cr' AND tmproyectos.fecha_generacion BETWEEN '$fecha_inicial' AND '$fecha_final' ORDER BY id_proyecto ASC",$conexion);
