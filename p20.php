  ...
  <tr>
    <td bgcolor="#FFFFFF" align="center"><br />
      <table width="900" border="0" cellspacing="0" cellpadding="4">
        <tr>
          <td align="center" class="titulo">Proyectos por Desarrollador conteo por Agente de Ventas</td>
        </tr>
        <tr>
...
$desarrolladores=mysql_query("SELECT DISTINCT tcusuarios.nombre, tcusuarios.id_usuario
FROM tcusuarios
JOIN tmproyectos
WHERE tcusuarios.id_usuario = tmproyectos.id_usuasignado ORDER BY tcusuarios.nombre ASC",$conexion);
while($fila=mysql_fetch_array($desarrolladores)){
	$ds = $fila['id_usuario'];
	$proyectos = mysql_query("
	SELECT DISTINCT tmproyectos.id_usugenera, COUNT(tmproyectos.id_usugenera) AS numero_proyectos, tcusuarios.id_usuario, tcusuarios.nombre AS agente
	FROM tmproyectos
	JOIN tcusuarios
	WHERE tmproyectos.id_usuasignado='$ds' AND tmproyectos.id_usugenera = tcusuarios.id_usuario AND tmproyectos.fecha_generacion BETWEEN '$fecha_inicial' AND '$fecha_final' GROUP BY agente ORDER BY numero_proyectos DESC",$conexion);
...
if ($nproyectos=="0")
{
	...
	FROM tmproyectos
	WHERE tmproyectos.id_usuasignado='$ds' AND tmproyectos.fecha_generacion BETWEEN '$fecha_inicial' AND '$fecha_final'",$conexion);
	...
	$totalp = $filatotal['totalp'];
		  echo '<table width="600" border="0" align="center" cellpadding="4" cellspacing="0">
			<tr class="encabezado-tabla">
				<td width="300">Agente de Ventas</td>
